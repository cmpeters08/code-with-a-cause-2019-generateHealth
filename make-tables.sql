...
CREATE TABLE PROVIDERS (provider_id SERIAL PRIMARY KEY, provider_name VARCHAR(255) NOT NULL, phone VARCHAR(255) NOT NULL, description text, hours VARCHAR(255) NOT NULL, days_of_operation VARCHAR(7), last_verified DATE, is_active BOOLEAN, last_updated TIMESTAMP NOT NULL)

CREATE TABLE REFERRALS (referal_id SERIAL PRIMARY KEY, referer_id INTEGER REFERENCES PROVIDERS(provider_id) NOT NULL, creator_id INTEGER REFERENCES USERS(user_id) NOT NULL, referee_id INTEGER REFERENCES PROVIDERS(provider_id) NOT NULL, is_eligiable BOOLEAN, date_of_referal DATE, referral_notes TEXT, fufillment_stats VARCHAR(255), updater_id INTEGER REFERENCES USERS(user_id) NOT NULL, last_updated TIMESTAMP NOT NULL)
...
CREATE TABLE WAITLIST (waitlist_id SERIAL PRIMARY KEY, provider_id INTEGER REFERENCES PROVIDERS(provider_id) NOT NULL, creator_id INTEGER REFERENCES USERS(user_id) NOT NULL, date_of_waitlist DATE, eligiable_for VARCHAR(255), is_active BOOLEAN, updater_id INTEGER REFERENCES USERS(user_id) NOT NULL, last_updated TIMESTAMP NOT NULL)

CREATE TABLE CLASSES (provider_id INTEGER REFERENCES PROVIDERS(provider_id) NOT NULL, creator_id INTEGER REFERENCES USERS(user_id) NOT NULL, class_name VARCHAR(255), class_description TEXT, address_id INTEGER REFERENCES ADDRESSES(address_id) NOT NULL, date_of_class DATE, time_of_class TIME, current_attendee_count INTEGER, max_attendees INTEGER, class_status VARCHAR(255), is_active BOOLEAN, updater_id INTEGER REFERENCES USERS(user_id) NOT NULL, last_updated TIMESTAMP NOT NULL)

CREATE TABLE ELIGIBILITY_REQUIREMENTS (provider_id INTEGER REFERENCES PROVIDERS(provider_id), public_assistance BOOLEAN NOT NULL, mother_17_or_younger BOOLEAN NOT NULL, is_client BOOLEAN NOT NULL, client_of_other VARCHAR(255), trimester INTEGER, residency_requirement BOOLEAN NOT NULL, must_meet_all_criteria BOOLEAN NOT NULL, updater_id INTEGER REFERENCES USERS(user_id) NOT NULL, last_updated TIMESTAMP NOT NULL)
